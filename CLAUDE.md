...
`npm run test:run` (Vitest, ~600 tests aujourd'hui couvrant matching / deck / session / parties / personnalites / analytics (incluant Plausible data-domain === PROD_HOSTNAME apex) / themes / App / aria-labels (WORDMARK_HOME_LABEL + PAGE_HEADER_NAV_LABEL + externalLinkLabel) / brand-colors (incluant noscript bg sync) / Card / ChipTop1 / Cover (incluant LEGISLATURE_LABEL + TAGLINE h1 round-trip + VIEW_RESULT_LABEL) / Legal (incluant AN_OPEN_DATA_URL) / Methode (incluant prose const round-trip + MAX_POINTS_CLES_BULLETS + READING_PAGE_MAX_WIDTH) / MethodeSheet / Result (incluant LEGISLATURE_LABEL) / site-metadata (incluant TAGLINE + BRAND_NAME + LEGISLATURE + PROD_ORIGIN + PROD_HOSTNAME + icons + APP_LOCALE sync) / ErrorBoundary / FreshnessBanner / RetryError (incluant RETRY_FETCH_FAILED_MESSAGE) / Skeleton (incluant CARD_FACE_BOX_SHADOW round-trip) / compute-positions (incluant boundary via THRESHOLD) / sanity / deck-invariants / parse-summary (incluant MAX_POINTS_CLES_BULLETS + FALLBACK_*) / parse-top (incluant MAX_SHARE_CARD_BARS) / contact (incluant ERROR_REPORT_SUBJECT + PROD_HOSTNAME derivation) / nav-state / routes (incluant isAffinementMode) / scrutins / share (incluant performShare fallback chain + AbortError consent + SHARE_SOURCE_LINE) / text-cleanup / vote-feedback / an-cache / an-filter / an-groups / an-parse / an-personnalites / env (incluant ANTHROPIC_BATCHES_URL + ANTHROPIC_API_VERSION + ANTHROPIC_MODEL + ingest tuning consts + WEB_SEARCH_TOOL_VERSION + SUPABASE_UPDATE_BATCH_SIZE) / useModalA11y / useFreshnessOnce / useFlipCardA11y / AuditTrail / DeckStack / PartyRow / PersonnaliteRow / RankingOverlay / TopBar / Wordmark, doivent rester verts à chaque commit).
