...
`npm run test:run` (Vitest, ~582 tests aujourd'hui couvrant matching / deck / session / parties / personnalites / analytics (incluant Plausible data-domain sync) / themes / App / brand-colors (incluant noscript bg sync) / Card / ChipTop1 / Cover (incluant LEGISLATURE_LABEL + TAGLINE h1 round-trip) / Legal (incluant AN_OPEN_DATA_URL) / Methode (incluant prose const round-trip + MAX_POINTS_CLES_BULLETS + READING_PAGE_MAX_WIDTH) / MethodeSheet / Result (incluant LEGISLATURE_LABEL) / site-metadata (incluant TAGLINE + BRAND_NAME + LEGISLATURE + PROD_ORIGIN + icons + APP_LOCALE sync) / ErrorBoundary / FreshnessBanner / RetryError (incluant RETRY_FETCH_FAILED_MESSAGE) / Skeleton (incluant CARD_FACE_BOX_SHADOW round-trip) / compute-positions (incluant boundary via THRESHOLD) / sanity / deck-invariants / parse-summary (incluant MAX_POINTS_CLES_BULLETS + FALLBACK_*) / parse-top (incluant MAX_SHARE_CARD_BARS) / contact (incluant ERROR_REPORT_SUBJECT) / nav-state / routes (incluant isAffinementMode) / scrutins / share / text-cleanup / vote-feedback / an-cache / an-filter / an-groups / an-parse / an-personnalites / env (incluant ANTHROPIC_BATCHES_URL + ANTHROPIC_API_VERSION + ANTHROPIC_MODEL + ingest tuning consts + WEB_SEARCH_TOOL_VERSION + SUPABASE_UPDATE_BATCH_SIZE) / useModalA11y / useFreshnessOnce / useFlipCardA11y / AuditTrail / DeckStack / PartyRow / PersonnaliteRow / RankingOverlay / TopBar / Wordmark, doivent rester verts à chaque commit).
