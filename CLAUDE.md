...
`npm run test:run` (Vitest, ~542 tests aujourd'hui couvrant matching / deck / session / parties / personnalites / analytics / themes / App / Card / ChipTop1 / Cover (incluant LEGISLATURE_LABEL) / Legal (incluant AN_OPEN_DATA_URL) / Methode (incluant prose const round-trip + MAX_POINTS_CLES_BULLETS) / MethodeSheet / Result (incluant LEGISLATURE_LABEL) / ErrorBoundary / FreshnessBanner / RetryError (incluant RETRY_FETCH_FAILED_MESSAGE) / Skeleton / compute-positions (incluant boundary via THRESHOLD) / sanity / deck-invariants / parse-summary (incluant MAX_POINTS_CLES_BULLETS + FALLBACK_*) / parse-top / contact / nav-state / routes (incluant isAffinementMode) / scrutins / share / text-cleanup / vote-feedback / an-cache / an-filter / an-groups / an-parse / an-personnalites / env (incluant ANTHROPIC_BATCHES_URL + ANTHROPIC_API_VERSION + ANTHROPIC_MODEL + ingest tuning consts) / useModalA11y / useFreshnessOnce / useFlipCardA11y / AuditTrail / DeckStack / PartyRow / PersonnaliteRow / RankingOverlay / TopBar / Wordmark, doivent rester verts à chaque commit).
