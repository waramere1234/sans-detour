...
`npm run test:run` (Vitest, ~513 tests aujourd'hui couvrant matching / deck / session / parties / personnalites / analytics / themes / App / Card / ChipTop1 / Cover (incluant LEGISLATURE_LABEL) / Legal / Methode (incluant prose const round-trip + MAX_POINTS_CLES_BULLETS) / MethodeSheet / Result (incluant LEGISLATURE_LABEL) / ErrorBoundary / FreshnessBanner / Skeleton / compute-positions (incluant boundary via THRESHOLD) / sanity / deck-invariants / parse-summary / parse-top / contact / nav-state / routes (incluant isAffinementMode) / scrutins / share / text-cleanup / vote-feedback / an-filter / an-groups / an-parse / an-personnalites / env / useModalA11y / useFreshnessOnce / useFlipCardA11y / AuditTrail / DeckStack / PartyRow / PersonnaliteRow / RankingOverlay / TopBar / Wordmark, doivent rester verts à chaque commit).
