...
import {
  RetryError, RETRY_FETCH_FAILED_MESSAGE, RETRY_DEFAULT_LABEL,
} from "../src/components/RetryError";
import { VIEW_RESULT_LABEL } from "../src/types";
...
describe("RetryError", () => {
  ...
  it("uses RETRY_DEFAULT_LABEL as the default button label", () => {
    render(<RetryError message="msg" onRetry={vi.fn()} />);
    expect(screen.getByRole("button", { name: RETRY_DEFAULT_LABEL })).toBeInTheDocument();
  });

  it("overrides the label via retryLabel (Play.tsx VIEW_RESULT_LABEL branch)", () => {
    render(<RetryError message="msg" onRetry={vi.fn()} retryLabel={VIEW_RESULT_LABEL} />);
    expect(screen.getByRole("button", { name: VIEW_RESULT_LABEL })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: RETRY_DEFAULT_LABEL })).not.toBeInTheDocument();
  });
